...
<header class="dashboard_header">
    <div class="dashboard_header_title">
        <h1 class="icon-search">Pesquisar Imóveis</h1>
...
<div class="dashboard_content">
    ...
    $Paginator = new Pager('dashboard.php?wc=imobi/search&pg=', '<<', '>>', 5);
    $Paginator->ExePager($Page, 12);

    $Search = filter_input(INPUT_GET, 's');
    $Read->ExeRead(DB_IMOBI, "WHERE realty_ref = :s OR realty_title LIKE '%' :s '%' ORDER BY realty_date DESC LIMIT :limit OFFSET :offset", "limit={$Paginator->getLimit()}&offset={$Paginator->getOffset()}&s={$Search}");
    if (!$Read->getResult()):
        ...
        $Paginator->ExePaginator(DB_IMOBI, "WHERE realty_ref = :s OR realty_title LIKE '%' :s '%'", "s={$Search}");
