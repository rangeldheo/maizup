...
$WhereCourse = null;
if ($OrderClause):
    $WhereCourse = "WHERE (order_id = '{$OrderClause}' OR order_transaction = '{$OrderClause}')";
...
<header class="dashboard_header">
    <div class="dashboard_header_title">
        ...
        <p class="dashboard_header_breadcrumbs">
            ...
            <span class="crumb">/</span>
            <?= ($OrdersEadStatus ? "Transações com status " . getWcHotmartStatus($OrdersEadStatus) : 'Transações'); ?>
        </p>
    </div>
...
<div class="dashboard_content">
    <section class="box_content">
        ...
        $Read->ExeRead(DB_EAD_ORDERS, "{$WhereCourse} ORDER BY order_purchase_date DESC LIMIT :limit OFFSET :offset", "limit={$Pager->getLimit()}&offset={$Pager->getOffset()}");
        if (!$Read->getResult()):
            $Pager->ReturnPage();
            ...
            $Pager->ExePaginator(DB_EAD_ORDERS, "{$WhereCourse}");
