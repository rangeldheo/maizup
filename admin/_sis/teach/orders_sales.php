...
if ($Search && ($Search['s'] || isset($Search['status']))):
    ...
    header("Location: dashboard.php?wc=teach/orders_sales&status={$Status}&s={$S}");
    exit;
endif;
?>
...
<header class="dashboard_header">
    ...
    <div class = "dashboard_header_search">
        ...
        <a title = "Recarregar Pedidos" href = "dashboard.php?wc=teach/orders_sales&status=<?= $OrdersEadStatus; ?>" class = "btn btn_green icon-spinner11 icon-notext"></a>
    </div>
</header>
...
<div class="dashboard_content">
    <div class="panel">
        ...
        $Pager = new Pager("dashboard.php?wc=teach/orders_sales&status={$OrdersEadStatus}&page=", "<", ">", 3);
        $Pager->ExePager($Page, 15);

        $Read->ExeRead(DB_EAD_ORDERS, "WHERE order_signature IS NULL {$WhereCourse} ORDER BY order_purchase_date DESC LIMIT :limit OFFSET :offset", "limit={$Pager->getLimit()}&offset={$Pager->getOffset()}");
        if (!$Read->getResult()):
            $Pager->ReturnPage();
            echo "<div class='trigger trigger_info trigger_none icon-info al_center font_medium'>Ainda não existem vendas!</div><div class='clear'></div>";
        else:
            ...
            $Pager->ExePaginator(DB_EAD_ORDERS, "WHERE order_signature IS NULL {$WhereCourse}");
